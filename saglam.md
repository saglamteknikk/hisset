...
<body>
    ...
    <script>
        ...
        // OTONOM KARAKTER DÜZELTME (Sihirli Kısım Burası)
        function fixText(el) {
            const trMap = {
            ...
            for(let i=0; i < val.length; i++) {
                let char = val[i];
                // Eğer harf Türkçe tablosundaysa değiştir, değilse olduğu gibi bırak
                corrected += trMap[char] || char;
            }

            // Hepsini büyük harfe çevir ve sadece A-Z, 0-9 ve boşluklara izin ver
            el.value = corrected.toUpperCase().replace(/[^A-Z0-9\s]/g, '');
        }
        ...
        async function gonder() {
            ...
            // Format: M:MESAJ|S:12:30|T:27.02.2026.5
